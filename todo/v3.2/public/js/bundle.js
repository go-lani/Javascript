...
var todos = [];
var category = 'all'; // 선택된 type의 상태 값, 'all' & 'active' & 'completed'
// DOMs

var $todos = document.querySelector('.todos');
var $input = document.querySelector('.input-todo');
var $completedAll = document.querySelector('#ck-complete-all');
...
var todoCount = function todoCount() {
  return todos.length > 0 ? Math.max.apply(Math, _toConsumableArray(todos.map(function (todo) {
    return todo.id;
  }))) + 1 : 1;
};
...
var render = function render() {
  ...
  var _todos = todos.filter(function (_ref) {
    ...
    return category === 'all' ? true : category === 'active' ? !completed : completed;
  });
...
var addTodo = function addTodo(content) {
  axios.post('/todos', content).then(function (res) {
    return todos = res.data;
  }).then(render)["catch"](function (err) {
    return console.error(err);
  });
...
var changeComplete = function changeComplete(id) {
  axios.patch("/todos/".concat(id)).then(function (res) {
    return todos = res.data;
  }).then(render)["catch"](function (err) {
    return console.error(err);
  });
...
var completeAll = function completeAll(completed) {
  axios.patch('/todos', {
    completed: completed
  }).then(function (res) {
    return todos = res.data;
...
var clearCompleted = function clearCompleted() {
  axios["delete"]('/clearCompleted').then(function (res) {
    return todos = res.data;
  }).then(render)["catch"](function (err) {
    return console.error(err);
  });
...
$completedAll.onclick = function (_ref6) {
...
$nav.onclick = function (_ref7) {
  ...
  _toConsumableArray($nav.children).forEach(function ($item) {
    return $item.classList.toggle('active', $item === target);
  });
